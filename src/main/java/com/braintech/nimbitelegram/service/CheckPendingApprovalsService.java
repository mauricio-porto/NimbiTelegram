...
//    @Scheduled(cron = "0 */3 * * * MON-FRI")
    ...
    private void savePendingPurchaserOrders(List<PurchaseOrderDetailedDTO> purchaseOrderDetailedDTOList) {
        ...
        pendingPurchaseOrdersRepository.deleteAll().block(Duration.ofMillis(2000L));

        Flux.fromStream(
                purchaseOrderDetailedDTOList.stream().map(purchaseOrderDetailedDTO -> {
                    return pendingPurchaseOrdersRepository.save(new PendingPurchaseOrders(idSupplier.get(), new Date(), purchaseOrderDetailedDTO));
                })
